...
function updatePackageJson(pkg) {
  ...
  const hasIndex = existsSync(join(pkg.path, 'src/index.ts')) || 
                   existsSync(join(pkg.path, 'src/index.tsx'));
  ...
  const projectJson = {
    ...
    targets: {
      build: {
        ...
        cache: true,
        dependsOn: workspaceDeps.length > 0 
          ? workspaceDeps.map(dep => `${dep}:build`)
          : undefined
      },
      test: {
        executor: 'nx:run-commands',
